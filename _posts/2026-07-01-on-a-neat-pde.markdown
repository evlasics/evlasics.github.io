...
---
I'm currently doing some research on the PDE $$ u_t - u_{xx} = \frac{u}{\lvert u\rvert^2} $$ where $$ u \colon \mathbb R \to \mathbb R^2 $$. This arises from physical scenarios, whatever. The interesting bit comes when we look at stationary solutions. These are solutions for which $$ u_t \equiv 0 $$ or in other words $$ u_{xx} = -u/\lvert u \rvert^2 $$. This sort of ODE will arise in physical contexts and is well studied because it is a prototypical example of a system in classical mechanics (if you replace the 2 with a 3 you get gravity).
